...
export class SteamCMDInterface {
  ...
  constructor() {
    ...
    this.fileName =
      this.downloadLink.url.split("/")[
        this.downloadLink.url.split("/").length - 1
      ]
  }
  ...
  async downloadCMD() {
    ...
    if (!this.downloadLink)
      throw new Error("You are running an unsupported platform.")
    this.cmd = path.join(rootFolder, "steamcmd" + this.downloadLink.ext)

    // check if steamCMD is downloaded
    const steamCmdExists = existsSync(
      path.join(rootFolder, `steamcmd${this.downloadLink.ext}`)
    )
    if (!steamCmdExists) {
