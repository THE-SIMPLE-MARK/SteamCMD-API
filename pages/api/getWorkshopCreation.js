...
async function handler(req, res) {
  if (req.method !== "GET") return res.status(405).send({ message: "Only get can be used in this endpoint." })
  ...
  try {
    ...
    const { dimensionX, dimensionY, dimensionZ, illegalBlocks } = await validateXML(vehicleXML);
    ...
    res.status(200).send({ message: "OK", dimensionX, dimensionY, dimensionZ, sizeCategory, illegalBlocks })
  } catch (err) {
...
function categorizeSize(dimX, dimY, dimZ) {
  ...
  if (dimX <= workbenchSizes.TRAIN_DEPOT.dimensionX && dimY <= workbenchSizes.TRAIN_DEPOT.dimensionY && dimZ <= workbenchSizes.TRAIN_DEPOT.dimensionZ) return "TRAIN_DEPOT"
  if (dimX <= workbenchSizes.SMALL_DOCK.dimensionX && dimY <= workbenchSizes.SMALL_DOCK.dimensionY && dimZ <= workbenchSizes.SMALL_DOCK.dimensionZ) return "SMALL_DOCK"
  if (dimX <= workbenchSizes.SMALL_HANGAR.dimensionX && dimY <= workbenchSizes.SMALL_HANGAR.dimensionY && dimZ <= workbenchSizes.SMALL_HANGAR.dimensionZ) return "SMALL_HANGAR"
  if (dimX <= workbenchSizes.MEDIUM_HANGAR.dimensionX && dimY <= workbenchSizes.MEDIUM_HANGAR.dimensionY && dimZ <= workbenchSizes.MEDIUM_HANGAR.dimensionZ) return "MEDIUM_HANGAR"
  if (dimX <= workbenchSizes.MEDIUM_DOCK.dimensionX && dimY <= workbenchSizes.MEDIUM_DOCK.dimensionY && dimZ <= workbenchSizes.MEDIUM_DOCK.dimensionZ) return "MEDIUM_DOCK"
  // if bigger than all
  return "HUGE"
}
...
async function validateXML(XMLObj) {
    ...
    Object.entries(obj).forEach(([key, value]) => {
      if (typeof value === "object") {
        ...
        if (illegalProperties.hasOwnProperty(key)) {
          ...
          if (!threshold(value, illegalPropertyRules.min, illegalPropertyRules.max)) {
            illegalBlocks.push({ property: key, value, expected: { min: illegalPropertyRules.min, max: illegalPropertyRules.max } })
          }
        }

        // only run size checks for this key if the value is a number
        if (!isNaN(value) && (key === "@_x" || key === "@_y" || key === "@_z")) {
          // run size checks
          if (key === "@_x") {
            numX++
  ...
  if (!smallestX || (numX < numY || numX < numZ) && smallestX > 0) smallestX = 0

  if (!largestY || (numY < numX || numY < numZ) && largestY < 0) largestY = 0
  if (!smallestY || (numY < numX || numY < numZ) && smallestY > 0) smallestY = 0

  if (!largestZ || (numZ < numX || numZ < numY) && largestZ < 0) largestZ = 0
  if (!smallestZ || (numZ < numX || numZ < numY) && smallestZ > 0) smallestZ = 0
